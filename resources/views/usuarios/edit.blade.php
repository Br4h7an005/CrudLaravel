...
<form id="form" action="{{ route('usuarios.update',$datos->id) }}" method="POST">
    ...
    <div class="row mt-4">
        <div class="col-md-4">
            <input type="text" name="telefono" class="form-control" placeholder="Ingrese teléfono" value="{{ old('telefono',$datos->telefono) }}">
            @error('telefono')
                <div class="error compacto col-lg-5">{{ $message }}</div>
            @enderror
        </div>
    ...
    <div class="row mt-4">
        <div class="col-md-4">
            <input type="email" name="email" class="form-control" placeholder="Ingrese correo" value="{{ old('email', $datos->email) }}">
            @error('correo')
                <div class="error compacto col-lg-5">{{ $message }}</div>
            @enderror
        </div>
